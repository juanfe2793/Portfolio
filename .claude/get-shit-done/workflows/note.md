...
- **Global scope**: `/Users/juanfe/code/Portfolio/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/Users/juanfe/code/Portfolio/.claude/notes/`)
...
2. Glob `/Users/juanfe/code/Portfolio/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/Users/juanfe/code/Portfolio/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/Users/juanfe/code/Portfolio/.claude/notes/` — works in any directory
