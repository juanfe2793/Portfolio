...
This script retrieve information about backup vaults in all regions. Here's a brief overview of what
the script does:

- Defines a function called `list_backup_vaults_by_region` that takes a single argument,
  region_name. This function uses the boto3 library to create a client for the EC2 service in the
  specified region, and then uses the `list_backup_vaults` method to retrieve information about all
  backup vaults in that region.
