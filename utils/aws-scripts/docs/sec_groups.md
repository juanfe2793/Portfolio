...
This script retrieve information about Security Groups in all regions. Here's a brief overview of
what the script does:

- Defines a function called `describe_sec_group_by_region` that takes a single argument,
  region_name. This function uses the boto3 library to create a client for the EC2 service in the
  specified region, and then uses the `describe_dhcp` method to retrieve information about all
  Security Groups in that region.
