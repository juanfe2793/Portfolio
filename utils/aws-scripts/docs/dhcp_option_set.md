...
This script retrieve information about DHCP option sets in all regions. Here's a brief overview of
what the script does:

- Defines a function called `describe_dhcp_by_region` that takes a single argument, region_name.
  This function uses the boto3 library to create a client for the EC2 service in the specified
  region, and then uses the `describe_dhcp` method to retrieve information about all DHCP option
  sets in that region.
